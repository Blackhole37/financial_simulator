...
export const getSubjects = async () => {
  const response = await fetch(`${API_BASE_URL}/subjects_dummy`);
  if (!response.ok) {
...
export const getLectures = async () => {
  const response = await fetch(`${API_BASE_URL}/lecture_dummy`);
  if (!response.ok) {
...
export const getTests = async () => {
  const response = await fetch(`${API_BASE_URL}/test_dummy`);
  if (!response.ok) {
...
export const fetchChatbotResponse = async (message, userId = null) => {
  ...
  try {
    ...
    console.log(
      "DEBUG - Chatbot Request URL:",
      `${API_BASE_URL}/chatbot?${params.toString()}`
    );
    console.log("DEBUG - Chatbot Parameters:", {
      message,
    ...
    if (!response.ok) {
      console.error(`DEBUG - Server Error: Status ${response.status}`);
