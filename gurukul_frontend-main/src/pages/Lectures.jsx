...
import { useGetLecturesQuery } from "../api/lecturesApiSlice";
import { Play } from "lucide-react";

export default function Lectures() {
  const { data: lectures, isLoading, isError, error } = useGetLecturesQuery();
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-8">
          {lectures &&
            lectures.map((lecture, index) => (
              <a
                key={`lecture-${index}-${
                  lecture.title || Math.random().toString(36).substr(2, 9)
                }`}
